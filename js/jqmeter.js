...
(function($) {
	...
	$.jQMeter = function(elem, options) {
		...
		if(meterOrientation == 'vertical') {
			...
			$(elem).html('<div class="therm outer-therm vertical"><div class="therm inner-therm vertical"></div><span>' + total + '</span></div>');
			...
			$(elem).html('<div class="therm outer-therm"><div class="therm inner-therm"></div><span style="display:none;">' + total + '</span></div>');
			$(elem).children('.outer-therm').attr('style','width:' + width + ';height:' + height + ';background-color:' + bgColor);
		...
		if(displayTotal) {
			...
			$(elem).children('.outer-therm').children('span').show();
			$(elem).children('.outer-therm').children('span').css('padding', padding);
			...
			$({ Counter: 0 }).animate({ Counter: $(elem).children('.outer-therm').children('span').text() }, {
  				duration : counterSpeed,
  				easing : 'swing',
  				step : function() {
   					$(elem).children('.outer-therm').children('span').text(Math.ceil(this.Counter) + '%');
  				}
			});
		}

		//Add CSS
		$(elem).append('<style>.therm{height:30px;border-radius:5px;}.outer-therm{margin:20px 0;position:relative;}.outer-therm span {color: #999;display: inline-block;font-family: Trebuchet MS;font-size:5px;position:absolute;right:-35px;top:-3px;}.vertical.outer-therm span{width:100%;text-align:center;}.vertical.outer-therm{position:relative;}.vertical.inner-therm{position:absolute;bottom:0;}</style>');
